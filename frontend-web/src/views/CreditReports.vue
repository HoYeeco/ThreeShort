    ...
    <el-dialog v-model="showSubmitDialog" title="提交信用行为上报" width="600px">
      <el-form :model="submitForm" label-width="100px">
        ...
        <el-form-item label="涉及人员">
          <el-select 
            v-model="submitForm.reportedUserId" 
            placeholder="请选择涉及人员（可选，如果是上报他人行为则必选）"
            style="width: 100%"
            clearable
          >
            <el-option 
          ...
          <div class="form-tip">
            • 如果上报自己的行为：不选择涉及人员，信用分记录到您的账户<br/>
            • 如果上报他人的行为：选择涉及人员，信用分记录到该人员账户
          </div>
        </el-form-item>
...
const submitForm = reactive({
  ...
  reportedUserId: null as number | null
})
...
const submitReport = async () => {
  try {
    ...
    const submitData = {
      ...submitForm,
      // 如果选择了涉及人员，将其姓名存储到involvedPersons字段（用于显示）
      involvedPersons: submitForm.reportedUserId ? 
        availableUsers.value.find(user => user.id === submitForm.reportedUserId)?.realName || '' : ''
    }
    ...
    submitForm.reportedUserId = null
    fileList.value = []
    loadData()
  } catch (error) {
