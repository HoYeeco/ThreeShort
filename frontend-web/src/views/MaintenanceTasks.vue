  ...
  <div class="maintenance-tasks">
    ...
    <el-card class="task-card">
      <template #header>
        <div class="card-header">
          <span>周期性信用分转化</span>
          <el-tag type="info">手动执行</el-tag>
        </div>
      </template>
      <div class="task-content">
        <p class="task-description">
          将用户信用分按规则转化为奖励积分并重置信用分：
        </p>
        <ul class="rule-list">
          <li>超过100分的部分：100%转化为奖励积分</li>
          <li>70-100分的部分：50%转化为奖励积分</li>
          <li>超过70分的信用分重置为70分</li>
          <li>小于70分的信用分保持不变</li>
        </ul>
        <div class="task-actions">
          <!-- <el-button 
            type="primary" 
            @click="executePeriodicScoreConversion"
            :loading="taskLoading.periodicConversion"
          ...
          <el-button 
            type="warning" 
            @click="forceExecutePeriodicScoreConversion"
            :loading="taskLoading.forcePeriodicConversion"
            :disabled="isExecuting">
...
import { Timer, Refresh, CircleCheck, Clock, DataLine, Money, Operation, RefreshRight, Warning } from '@element-plus/icons-vue'
...
const syncLoading = ref({
  ...
  periodicScoreConversion: false,
  forcePeriodicConversion: false
})

// 任务加载状态
const taskLoading = ref({
  periodicConversion: false,
  forcePeriodicConversion: false
})

// 是否有任务正在执行
const isExecuting = computed(() => {
  return Object.values(syncLoading.value).some(loading => loading) ||
         Object.values(taskLoading.value).some(loading => loading)
})
...
// 执行周期性信用分转化
const executePeriodicScoreConversion = async () => {
  try {
    await ElMessageBox.confirm(
      '确定要执行周期性信用分转化吗？这将为所有用户生成当前周期的信用分转化记录。',
      '确认操作',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
    ...
    taskLoading.value.periodicConversion = true
    
    await request.post('/system/tasks/execute/periodic-score-conversion')
    
    ElMessage.success('周期性信用分转化执行成功！')
    ...
    taskLoading.value.periodicConversion = false
  }
}

// 强制重新执行周期性信用分转化
const forceExecutePeriodicScoreConversion = async () => {
  try {
    await ElMessageBox.confirm(
      '确定要强制重新执行周期性信用分转化吗？这将为所有用户重新生成当前周期的信用分转化记录。',
      '确认操作',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
    ...
    taskLoading.value.forcePeriodicConversion = true
    
    await request.post('/system/tasks/execute/periodic-score-conversion/force')
    
    ElMessage.success('强制重新执行周期性信用分转化成功！')
    ...
    taskLoading.value.forcePeriodicConversion = false
  }
}
