...
public class TaskManagementController {
    ...
    @Operation(summary = "手动执行周期性信用分转化", description = "手动触发周期性信用分转化，将超过条件的信用分转化为奖励积分并重置")
    @PostMapping("/execute/periodic-score-conversion")
    @RequireRole({User.UserRole.MAINTAINER})
    public Result<String> executePeriodicScoreConversion() {
        try {
            log.info("手动执行周期性信用分转化");
            
            // 执行周期性信用分转化
            creditScoreRecordService.executePeriodicScoreConversion();
            
            // 记录任务执行
            recordTaskExecution("PERIODIC_SCORE_CONVERSION", "SUCCESS", "手动执行成功");
            
            return Result.success("周期性信用分转化完成");
        } catch (Exception e) {
            log.error("手动执行周期性信用分转化失败", e);
            recordTaskExecution("PERIODIC_SCORE_CONVERSION", "FAILED", e.getMessage());
            return Result.error("执行失败: " + e.getMessage());
        }
    }

    @Operation(summary = "强制重新执行周期性信用分转化", description = "强制重新执行周期性信用分转化，忽略已存在的记录，重新计算所有用户的转化")
    @PostMapping("/execute/periodic-score-conversion/force")
    @RequireRole({User.UserRole.MAINTAINER})
    public Result<String> forceExecutePeriodicScoreConversion() {
        try {
            log.info("强制重新执行周期性信用分转化");
            
            // 强制重新执行周期性信用分转化
            if (creditScoreRecordService instanceof com.community.credit.service.impl.CreditScoreRecordServiceImpl) {
                ((com.community.credit.service.impl.CreditScoreRecordServiceImpl) creditScoreRecordService)
                    .executePeriodicScoreConversion(true);
            } else {
                return Result.error("服务实现类型不匹配，无法执行强制重新执行");
            }
            
            // 记录任务执行
            recordTaskExecution("PERIODIC_SCORE_CONVERSION_FORCE", "SUCCESS", "强制重新执行成功");
            
            return Result.success("强制重新执行周期性信用分转化完成");
        } catch (Exception e) {
            log.error("强制重新执行周期性信用分转化失败", e);
            recordTaskExecution("PERIODIC_SCORE_CONVERSION_FORCE", "FAILED", e.getMessage());
            return Result.error("执行失败: " + e.getMessage());
        }
    }
